...
def DIJ(max_node):
    ...
    weighted_distance[0][0] = 0                 #start node level initializing
    weighted_distance[0][1] = 0                 #start node weighted distance initializing

    level = 1
    while True:
        count = 0
        for i in range(max_node):                                             #from 0 to max_node-1 (1 to max_node)
...
answer = [0]
for pair in DIJ(max_node)[1:]:
    if pair[0] == -1:
        answer.append(pair[0])
    else:
        answer.append(pair[1])

w = open('output_dij.txt', 'w')
